  ...
  return (
    <div className="fixed inset-0 pointer-events-none z-[9999] overflow-hidden hidden lg:block">
      <motion.div
        ...
        className="w-8 h-8 rounded-full border-2 border-white/20 absolute top-0 left-0"
      />
      <motion.div
        style={{ x: sx, y: sy, translateX: '-50%', translateY: '-50%' }}
        animate={{ 
        ...
        className="w-1.5 h-1.5 bg-white rounded-full absolute top-0 left-0 shadow-[0_0_10px_rgba(255,255,255,0.5)]"
      />
      <motion.div
        style={{ x: sx, y: sy, translateX: '-50%', translateY: '-50%' }}
        animate={{ opacity: isHovering ? 0.4 : 0 }}
        className="w-20 h-20 bg-blue-500/20 rounded-full blur-3xl absolute top-0 left-0"
      />
    </div>
  );
}
